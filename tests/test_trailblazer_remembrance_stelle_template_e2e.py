"""开拓者•记忆 8008（星）模板端到端对轴（验收型批）：真模板 YAML → 编译 →
...
MEM_HP = 1047.816 * 0.8 + 640        # 1478.2528（800804 lv10 #2/#4 烘焙）
MEM_SPD = 130.0
...
def _build(*, eidolon: int = 0, pre_battle: bool = False, ally=None, extra_template=None):
    member = {"character_template": "8008", "level": 80}
    if eidolon:
    ...
    if pre_battle:
        build["build"]["pre_battle"] = [{"actor_id": "8008", "technique": "800807"}]
    return build
...
def _tb(eng):
    return eng.state.actors["8008"]


def _mem(eng):
    return eng.state.actors["8008_mem"]
...
def _cast(eng, aid, *, target="e1"):
    ...
    st = _tb(eng)
    a = next(x for x in eng.actions_by_actor["8008"] if x.action_id == aid)
    tgt = eng.state.actors[target]
    eng.decision.select_target = lambda actor_state, action_type, candidates, engine: (
        tgt if tgt in candidates else (candidates[0] if candidates else None))
    eng._execute_action(st, a)
    eng.bus.emit("on_action", {
        "actor": "8008", "action_type": a.action_type, "action_id": aid,
...
def _mem_cast(eng, aid, *, target="e1"):
    ...
    st = _mem(eng)
    a = next(x for x in eng.actions_by_actor["8008_mem"] if x.action_id == aid)
    tgt = eng.state.actors[target]
    eng.decision.select_target = lambda actor_state, action_type, candidates, engine: (
        tgt if tgt in candidates else (candidates[0] if candidates else None))
    eng._execute_action(st, a)
    eng.bus.emit("on_action", {
        "actor": "8008_mem", "action_type": a.action_type, "action_id": aid,
        "target_type": a.target_type, "target": tgt.actor.actor_id,
        "actor_type": st.actor.actor_type}, eng.state)
...
def _ult(eng):
    ...
    ult = next(a for a in eng.actions_by_actor["8008"] if a.action_id == "800803")
...
class TestTrailblazerCompile:
    def test_actions_summon_resources(self, compiled):
        acts = {a.action_id for a in compiled.actions_by_actor["8008"]}
        assert acts == {"800801", "800802", "800809", "800803", "800808"}
        mem_acts = {a.action_id for a in compiled.actions_by_actor["8008_mem"]}
        assert mem_acts == {"1800701", "1800707"}
        decls = compiled.resource_decls_by_actor["8008"]
        assert decls["epic"]["max"] == 2
        assert compiled.resource_decls_by_actor["8008_mem"]["charge"]["max"] == 1, (
            "充能忆灵自持（12_summon v1.2——重召布场重置、不在场不累计）")
        sd = compiled.summon_defs["8008_mem"]
        ...
        assert math.isclose(sd.actor.stats.spd, MEM_SPD), "迷迷初速 130（800804 #1）"
...
class TestTracesAndBattleStart:
    def test_trace_stats_and_advance30(self, compiled):
        ...
        assert math.isclose(_remaining(eng, "8008"), 7000.0), "10000-3000（距离制——拉条 30%）"


class TestSkillSummonAndRestore:
    def test_summon_branch_full_pack(self, compiled):
        """800802：耗 1 点回 30 能；召唤迷迷（继承白值+烘焙 hp/spd）；充能 0.5+0.4=0.9
        ...
        assert _remaining(eng, "8008_mem") > 0.0, "90% 未满——不立即行动"
        ...
        assert math.isclose(eng.pipeline.effective_stats(eng.state.actors["ally"])["crit_dmg"],
                            0.5 + AURA_CD), "辅手同吃光环"
        assert _legal(eng) == {"800801", "800809", "800803"}, "召唤后 800802 锁、800809 开"

    def test_restore_branch_heal_and_charge(self, compiled):
        """800809（迷迷在场）：治疗 0.6×迷迷上限 + 充能 +0.1（+天赋 0.03）——0.9+0.1+0.03
        触 100% 立即行动."""
        eng = _make(compiled)
        _summon(eng)
        mem = _mem(eng)
        ...
        assert math.isclose(_remaining(eng, "8008_mem"), 0.0), "1800703：充能达 100% 立即行动"
...
class TestUltimateDoubleBranch:
    def test_first_summon_e0u(self, compiled):
        ...
        assert math.isclose(_remaining(eng, "8008_mem"), 0.0), "满充能立即行动"
        assert math.isclose(st.resources["epic"], 1.0), "8008501：终结技后 +1 层史诗"
...
class TestTalentEnergyToCharge:
    def test_energy_waterfall(self, compiled):
        """800804：我方每回 10 能 → 迷迷 +1% 充能（// 池守恒）。本体普攻 1.0 档伤害对轴
        + 充能三步（辅手回能同计——全队累计口径）."""
        eng = _make(compiled)
        _summon(eng)
        rec = _hits(eng)
        _cast(eng, "800801")                    # 本体 +20 能 → +0.02
...
class TestEnhancedBasicJoint:
    def test_epic_consume_purify_and_joint(self, compiled):
        """800808：耗 1 史诗+净化迷迷控制件；连携双半=本体 1.2×619.38 / 迷迷 1.2×543.31
        （lv6 档，各自面板）；充能 +0.1（+天赋 0.03）；削韧 5+5；互斥翻转."""
        eng = _make(compiled)
        _summon(eng)
        _ult(eng)                               # 史诗 1、充能 1.0
        _support(eng, "ally")                   # 充能归零（声援落辅手——本例不再行动）
        assert _legal(eng) == {"800809", "800803", "800808"}, "史诗+在场 → 强化普攻开、普攻锁"
        mem = _mem(eng)
        ...
        tb_half = [h for h in rec if h.get("action_type") == "basic" and h["source"] == "8008"]
        mem_half = [h for h in rec if h.get("action_type") == "basic" and h["source"] == "8008_mem"]
        ...
        assert _legal(eng) == {"800801", "800809", "800803"}, "史诗耗尽 → 普攻回换 800801"
...
class TestMemospriteSkill:
    def test_baddies_bounce_and_aoe(self, compiled):
        ...
        assert math.isclose(_mem(eng).resources["charge"], 0.9 + 0.05), "8008102：施放 +5% 充能"
        ...
        assert _legal(eng, "8008_mem") == {"1800701"}, "充能未满：支援锁"
...
class TestMemospriteSupport:
    def test_advance_support_charge_reset_and_true_dmg(self, compiled):
        ...
        assert _legal(eng, "8008_mem") == {"1800707"}, "充能满：仅支援可点"
        _support(eng, "ally")
    ...
    def test_self_cast_no_advance(self, compiled):
        ...
        mem = _mem(eng)
        mem.resources["charge"] = 1.0
        _set_remaining(eng, "8008_mem", 4000.0)
        _support(eng, "8008_mem")
        assert "MEM_SUPPORT" in mem.modifiers
        assert math.isclose(_remaining(eng, "8008_mem"), 4000.0), "自身施放不拉条"

    def test_true_dmg_trace_scaling(self, compiled):
        """8008103 磁石与长链：本体（能量上限 160）持声援 → mem_boost 0.12 →
        真伤倍率 0.36+0.12=0.48."""
        eng = _make(compiled)
        _summon(eng)
        mem = _mem(eng)
...
class TestMemExit:
    def test_exit_pack(self, compiled):
        ...
        eng = _make(compiled)
        _summon(eng)
        _set_remaining(eng, "8008", 5000.0)
        assert eng.dismiss_summon_actor("8008_mem") is True
        assert math.isclose(eng.pipeline.effective_stats(eng.state.actors["ally"])["crit_dmg"],
                            0.5), "光环随灭摘除"
        assert math.isclose(_remaining(eng, "8008"), 2500.0), "1800706：行动提前 25%"
        assert _legal(eng) == {"800801", "800802", "800803"}, "迷迷离场 → 回复分支锁、召唤分支回"
...
class TestEidolons:
    def test_e1_propagation_and_crit(self):
        ...
        mem = _mem(eng)
        mem.resources["charge"] = 1.0
        _support(eng, "8008")                   # 声援落本体 → E1 同播迷迷
    ...
    def test_e2_other_memosprite_action_energy(self):
        ...
        eng.bus.emit("on_turn_start", {"actor": "8008"}, eng.state)
        _evey_act()
    ...
    def test_e3_skill_lv12_heal(self):
        """E3：战技+2 → 800809 治疗实取 lv12=0.66×迷迷上限."""
    ...
    def test_e5_ult_basic_lv_shift(self):
        ...
        rec = _hits(eng)
        _cast(eng, "800801")
        basic = [h for h in rec if h.get("action_type") == "basic" and h["source"] == "8008"]
        assert len(basic) == 1 and math.isclose(
    ...
    def test_e6_fixed_crit_scoped(self):
        ...
        assert math.isclose(_mem(eng).resources["charge"], 0.03 + 0.05 + 0.03), (
            "E4 联动：支援施放（迷迷=零能量上限放技）+3% → 归零后入账；8008102 +5% + 本击再放技 +3%")
...
class TestFullRunSmoke:
    def test_rule_policy_full_chain(self, compiled):
        ...
        assert _mem(eng).alive or state.actors["8008"].resources.get("epic", 0.0) >= 0.0
