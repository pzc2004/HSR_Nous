"""开拓者•欢愉 8009 模板端到端对轴（验收型批）：真模板 YAML → 编译 → 好活当赏条目账/
笑点账/天赋追加/终结技双分支/欢愉技段链/行迹/星魂全链 → 手算全等（过堂勘正十八件
见 fixture 头注；B40 迁移记六件同注）。8010 双子同套件——结构与口径对齐（e2e 文件名
带 cid 消歧）。
...
×elation_multi×punchline_multi×期望暴击区×0.5×0.9，见 `_el`。8009 实战 atk
596.09088<1000 → 行迹 8009101 欢愉度 +0 → elation_multi=1.0；punchline_multi=
1+5·src/(src+240)，src 定槽（21_elation §21.2）：欢愉技 800920 全段=阿哈笑点池实时值
...
闩退役，B40 ⓵）；800920 收尾均摊段=action split:even + scaling elation 行键单承
（勘正⑦——8010 双子为 hook 手算分母同数值解，原语口径差异在案待上游统一）。
"""
...
def _el(mult: float, src: float, cz: float) -> float:
    ...
    cz=期望暴击区（CZ 族）。elation_multi=1.0——面板欢愉度 0（atk<1000 行迹 8009101 不给）。
    """
...
class TestTrailblazerCompile:
    def test_actions_resources(self, compiled):
        ...
        assert acts["800920"].action_type == "elation_skill", "欢愉技 action_type 正身（B40 ⓵）"
    ...
    def test_elation_skill_not_in_legal_set(self, compiled):
        ...
        eng = _make(compiled)
        legal = legal_action_set(_tb(eng), eng.actions_by_actor["8009"], 5)
        assert "800920" not in {a.action_id for a in legal}
...
class TestUltimate:
    def test_self_target_branch_a(self, compiled):
        ...
        e2=0.3（split even 单承勘正⑦）；+5 笑点/回 1 点；代放的欢愉技回 5 能（tbgd）再计
        天赋 +10 能 +3 笑点；阿哈挂账."""
        eng = _make(compiled)
        st = _tb(eng)
        e1, e2 = eng.state.actors["e1"], eng.state.actors["e2"]
...
class TestElationSkillManual:
    def test_segments_manual_axis(self, compiled):
        """欢愉技 lv10（手动对轴——体系触发行动不走合法集，直施对轴，爻光同法）：8 段随机
        0.2（expected 确定化首敌）+ 均摊 0.6/2（split even），全段笑点池实时值（进战 1）；
    ...
    def test_aha_sic_em_bank_and_clear(self, compiled):
        """阿哈咬它（行迹 8009103）：普攻不挂账；辅手追加（follow_up）亦不挂账——旧
...
class TestEidolons:
    def test_e1_armed_bank(self):
        ...
        _ult(eng, "8009")   # 代放 800920 → 阿哈挂账 1
