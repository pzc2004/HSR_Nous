...
口径常数：娜塔莎白值 atk 476.28、hp 1164.24、crit 0.05/0.5（期望暴击区 1.025）；
...
class TestEidolons:
    def test_e1_self_heal_low_hp(self):
        """E1：受击且 HP≤30% → 自疗 (0.15×HP+400)×(1+0.1+0.5 天赋命中域) 超上限
        clamp 满血，单场 1 次."""
        ...
        assert math.isclose(a.current_hp, NA_HP, rel_tol=1e-9), (
            "574.6×1.6=919.4 超顶 clamp（天赋 scoped 自身低血同吃）")
    ...
    def test_e2_ult_hot_and_e4_hit_energy(self):
        """E2：大招对 ≤30% 队友附加 HoT（同事件后位求值——治疗先落：本例 300→
        881.5 仍 ≤30% 挂得上；800→1381.5 抬过阈即不挂，官方未写时点双态在案）；
        E4：受击 +5 能."""
        compiled = compile_encounter(_build(eidolon=4), _STAGE, template_roots=TEST_TEMPLATE_ROOTS)
        eng = _make(compiled)
        ally = eng.state.actors["ally"]
        ally.current_hp = 50.0   # 治疗 (0.138×HP+368)×1.6(天赋命中域)=845.9 → 895.9 仍 ≤30%
        m7 = _na(eng)
        m7.current_energy = 90.0
        ult = next(a for a in eng.actions_by_actor["1105"] if a.action_id == "110503")
        assert eng._fire_ultimate(m7, ult) is True
    ...
    def test_e2_ordering_countercase(self):
        """E2 反例（时点钉死）：队友 800/3000=26.7% 开战低血，但同发治疗先抬到
        46% → 不挂（同事件后位求值——官方时点未写在案）."""
        compiled = compile_encounter(_build(eidolon=4), _STAGE, template_roots=TEST_TEMPLATE_ROOTS)
        eng = _make(compiled)
        ally = eng.state.actors["ally"]
