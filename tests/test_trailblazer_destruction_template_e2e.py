...
class TestTechnique:
    def test_pre_battle_heal(self):
        ...
        起手 current_hp=白值（引擎布场口径，8002 同案）：开拓者 1203.048 距有效上限
        1419.59664 有 216.55 头部空间 → 实回 212.939496=0.15×1419.59664；辅手
        白值=有效=3000 满血 → 拟回全转 excess=450."""
        ...
        assert math.isclose(got["8001"]["amount"], 0.15 * HP_EFF, rel_tol=1e-9), (
            "实回 0.15×自身有效上限（含行迹 hp_pct 0.18）")
