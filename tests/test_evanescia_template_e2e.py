"""绯英 1505 模板端到端对轴（验收型批·欢愉体系首租——B40 迁移版）：真模板 YAML →
编译 → 双向转化/狐狸老师/欢愉伤害真路由/弹射/行迹/星魂全链 → 手算全等（B40 迁移记
⓵-⓼ 见 fixture 头注——勘正⑧⑨⑫⑭ 翻案在案）.
...
欢愉伤害真路由（B40 ⓸——category elation 引擎消费 02 §2.14/08 §8.1）：7535.107(lv80)
×纯倍率×(1+elation面板)×(1+5X/(X+240))×期望暴击区×防御区×抗性区×0.9×易伤×(1+增笑)；
elation 面板=行迹欢愉度强化 0.18+天赋 #5×暴伤（B40 ⓹——旧「欢愉增伤 1.18 折入」误读
翻案：E0 综合乘区 1.298→1.28、E2 1.383→1.352）；X=punchline_source 定槽——战技/狐狸
段=持有者好活当赏合并值、终结技系保底 max(合并值,480)、欢愉技=笑点池实时值。
好活当赏=引擎原生条目列表（st.banger_entries 逐条目独立计时，eng._resource_value 读
合并值——旧自制 banger 资源翻案，B40 ⓶）；笑点=队伍账 eng.state.punchline（B40 ⓷）；
欢愉技 150520 正身 action_type=elation_skill（合法行动集引擎排除手动施放——阿哈时刻
/E1 代放驱动，B40 ⓵⓺）；进战 20 好活当赏引擎统发先于 hook 订阅→转化由 ⓼ 镜像钩兑现
（energy/meter 各 +20——引擎序嫌疑在案）。
假人 def 1000 → 防御区 0.5（E4 起无视 15% → 1000/1850）；弱点匹配 → 抗性区 1.0
（E1 起抗穿 20% → 1.2）；未击破 0.9；期望暴击区 1+0.537×暴伤。
...
ATK = 737.352
CRIT_E0 = 1 + 0.537 * 0.5              # 1.2685（E0-1 期望暴击区）
CRIT_E2 = 1 + 0.537 * 0.86             # 1.46182（E2 起暴伤 0.86）
DEF_E0 = 0.5                            # 假人 def 1000
DEF_E4 = 1000 / (1000 * 0.85 + 1000)    # E4 无视 15% 防御 → 1000/1850
ELA_LVL = 7535.107                      # 欢愉伤害等级系数 lv80（rulebook elation_level_multiplier）
ELATION_E0 = 1 + 0.18 + 0.2 * 0.5       # elation 面板=行迹 0.18+天赋 #5×暴伤（B40 ⓹）→ 1.28
ELATION_E2 = 1 + 0.18 + 0.2 * 0.86      # → 1.352
...
def _ela(ratio, x, *, crit=CRIT_E0, defz=DEF_E0, res=1.0, elation=ELATION_E0,
         merry=1.0, vuln=1.0):
    return ELA_LVL * ratio * elation * _punch(x) * defz * res * 0.9 * vuln * crit * merry
...
def _banger(eng):
    return eng._resource_value(_fx(eng), "certified_banger")   # 条目列表合并值（B40 ⓶）
...
class TestCompile:
    def test_actions_resources_and_costs(self, compiled):
        acts = {a.action_id: a for a in compiled.actions_by_actor["1505"]}
        assert set(acts) == {"150501", "150502", "150503", "150520"}, (
            "欢愉技 150520 正身落 actions（B40 ⓵——勘正⑨翻案）")
        assert acts["150520"].action_type == "elation_skill", "欢愉技 action_type 正身（B40 ⓵）"
        assert math.isclose(acts["150520"].scaling[9]["elation"], 1.1), (
            "欢愉技 lv10 #2=1.1 纯倍率（elation 行键路由）")
        ult = acts["150503"]
        ...
        assert set(decls) == {"_fox_meter", "_e6_n", "banger_turns"}, (
            "自制 banger/punchline decl 摘除——条目列表/队伍账引擎原生（B40 ⓶⓷）")

    def test_elation_skill_not_in_legal_set(self, compiled):
        """欢愉技=体系触发行动（阿哈时刻/代放族）——合法行动集引擎排除手动施放（B40 ⓵）."""
        eng = _make(compiled)
        legal = legal_action_set(_fx(eng), eng.actions_by_actor["1505"], 5)
        assert "150520" not in {a.action_id for a in legal}
...
class TestPanelTrace:
    def test_trace_panel(self, compiled):
        ...
        assert math.isclose(eff["elation"], 0.18 + 0.2 * 0.5, rel_tol=1e-9), (
            "elation 面板=行迹欢愉度强化 0.18+天赋 #5×暴伤（B40 ⓹——旧 1.18 误读翻案）")


class TestBattleStart:
    def test_entry_grants(self, compiled):
        """进战 +20 好活当赏条目（引擎统发）+ 转化镜像 +20 能量/+20 累计（B40 ⓼）+
        每欢愉角色 +1 笑点（08 §8.1/§8.2）."""
        eng = _make(compiled)
        st = _fx(eng)
        assert math.isclose(_banger(eng), 20.0), "引擎 _init_aha 统发（合并值口径）"
        assert st.banger_entries == [{"value": 20.0, "turns": 2.0}], (
            "引擎原生条目列表——单条目 20 点/2 回合独立计时（B40 ⓶）")
        assert math.isclose(st.current_energy, 20.0), "好活当赏→能量转化（⓼ 镜像钩单步互锁）"
...
class TestBasic:
    def test_basic_damage_and_conversions(self, compiled):
        """普攻 lv6=1.0 ATK：737.352×1.0×0.5×0.9×1.2685；回能 20 → 好活当赏/累计 各 +20."""
        eng = _make(compiled)
        st = _fx(eng)
        h1 = _hp(eng, "e1")
        _cast(eng, "150501")
...
class TestSkill:
    def test_blast_and_elation_follow(self, compiled):
        ...
        （X=合并值——能量转化先于命中结算，on_action 时点合并值=50）."""
        eng = _make(compiled)
        st = _fx(eng)
    ...
    def test_banger_refill_via_conversion(self, compiled):
        """门控自愈链：条目清空开战技 → 回能 30 先转化 30 好活当赏 → 欢愉段照发（X=30）
...
class TestUltimate:
    def test_aoe_bounces_and_elation(self, compiled):
        ...
        assert math.isclose(_banger(eng), 25.0)
        assert math.isclose(st.resources["_fox_meter"], 25.0)

    def test_floor_480_with_empty_banger(self, compiled):
        """终结技保底（官方「至少计入等同于能量上限的【好活当赏】」——punchline_source
        max 表达式）：条目清空开大，回能 5 转化 5 → X=max(5,480)=480——欢愉段与
        合并值=25 时逐值全等."""
        eng = _make(compiled)
        st = _fx(eng)
        st.banger_entries.clear()
...
class TestFoxChain:
    def test_threshold_trigger_and_vuln(self, compiled):
        ...
        assert math.isclose(_banger(eng), 50.0), "20+20(转化)+10(狐狸回能转化)"
...
class TestConversionCaps:
    def test_cap_direction_and_lock(self, compiled):
        ...
        CONV_LOCK 互锁单步（转化产物不再转化——合并值 320 而非 420）."""
        eng = _make(compiled)
        st = _fx(eng)
        st.resources["_fox_meter"] = 0.0
        eng._grant_energy(st, 150.0, source="1505", action_id=None, reason="test")
        assert math.isclose(_banger(eng), 170.0), "20+150——能量侧等值无 cap"
        assert math.isclose(st.resources["_fox_meter"], 150.0)
        assert math.isclose(st.current_energy, 170.0)
        st.resources["_fox_meter"] = 0.0
        eng._gain_resource(st, "certified_banger", 150.0, source_id="1505")
        assert math.isclose(_banger(eng), 320.0), "不再二次转化（互锁单步）"
...
class TestTechnique:
    def test_pre_battle_loadout(self, compiled):
        """秘技：全体 100% ATK（削韧 20）+ 好活当赏 20 条目 → 与进战 20 合计 40/40/40."""
...
class TestElationSkill:
    def test_manual_axis(self, compiled):
        """欢愉技 lv10（手动对轴——体系触发行动不走合法集，直施对轴）：AoE #2=1.1 欢愉
        真路由（X=笑点池实时值=进战 1）→ 获得 #1=5 好活当赏（on_action 发放——文本序
        伤害在先获得在后）→ 转化 5 能/5 累计；不耗点不产点；削韧 0（待收⑪）."""
        eng = _make(compiled)
        st = _fx(eng)
        h1, h2 = _hp(eng, "e1"), _hp(eng, "e2")
        _cast(eng, "150520")
        for aid, h in (("e1", h1), ("e2", h2)):
            assert math.isclose(h - _hp(eng, aid), _ela(1.1, 1), rel_tol=1e-9), (
                "AoE 1.1 纯倍率×等级系数路由，X=池实时值 1")
        assert math.isclose(_banger(eng), 25.0), "进战 20 + 欢愉技 #1=5（独立条目合并加和）"
        assert math.isclose(st.current_energy, 25.0), "镜像 20 + 转化 5"
        assert math.isclose(st.resources["_fox_meter"], 25.0)
        assert math.isclose(eng.state.skill_points, 3.0), "不耗不产"
        assert math.isclose(eng.state.punchline, 1.0), "手动对轴不经阿哈清池"
        assert math.isclose(eng.state.actors["e1"].toughness, 100.0), "削韧保守 0（待收⑪）"


class TestE1:
    def test_res_pen_and_fox_elation_skill(self):
        """E1：抗穿 20%（抗性区 1.2）+ 狐狸攻击后 trigger_action 代放欢愉技（B40 ⓺——
        X=笑点池=1；行裁断易伤先入网吃 ×1.12；欢愉技 #1=5+E1 额外 10=15 好活当赏各
        自独立条目 → 各转化 15 能/累计）."""
        ...
        assert math.isclose(_banger(eng), 85.0)
...
class TestE4:
    def test_def_pen_and_levels(self):
        ...
        E3 普攻 lv7=1.1、期望暴击区 1+0.537×0.86."""
...
class TestE6Fox:
    def test_merrymake_scoped_and_rebake(self):
        """E6 增笑（merrymake 真键 stat_exprs 现场求值——B40 ⓻）：开战 0.154（合并值 20）；
        狐狸链中随合并值重估——狐狸欢愉段 m=1.158（40）、E1 代放欢愉技 m=1.16（50）且吃
        易伤；物理段不吃增笑（merrymakeMulti 是欢愉专属乘区，勘正⑫ 翻案）."""
        eng = _make(_compiled(eidolon=6))
        st = _fx(eng)
        assert math.isclose(eng.pipeline.effective_stats(st)["merrymake"],
                            0.15 + 0.02 * 0.2, rel_tol=1e-9), "合并值 20 → 0.154"
        st.resources["_fox_meter"] = 235.0
        h1 = _hp(eng, "e1")
        eng._grant_energy(st, 20.0, source="1505", action_id=None, reason="test")
        ...
        assert math.isclose(h1 - _hp(eng, "e1"), x, rel_tol=1e-9)
        assert math.isclose(eng.pipeline.effective_stats(st)["merrymake"],
                            0.15 + 0.02 * 0.65, rel_tol=1e-9), "合并值 65 → 0.163 重估"
        assert math.isclose(st.current_energy, 65.0)
        assert math.isclose(_banger(eng), 65.0)
        assert math.isclose(st.resources["_fox_meter"], 40.0)
        assert math.isclose(st.resources["banger_turns"], 3.0), (
            "E6 好活当赏持续 +1 回合（banger_turns 2→3，B40 ⓻）")


class TestE6Ult:
    def test_ult_lv12_and_first_cast_energy(self):
        """终结技 lv12（E3）：全体 1.76 + 弹射 7×1.296；天赋 lv12（E5）：欢愉 #6=0.264/
        #8=0.308（X 保底 480；增笑 m=1.155——合并值 25 时点）；E6 首次终结技固定回 120
        （err_exempt）→ 好活当赏 +120（无 cap——能量侧）→ 增笑重估 0.179."""
        eng = _make(_compiled(eidolon=6))
        st = _fx(eng)
        h1, h2 = _hp(eng, "e1"), _hp(eng, "e2")
        _ult(eng)
        ...
        assert math.isclose(_banger(eng), 145.0), "20+5+120"
