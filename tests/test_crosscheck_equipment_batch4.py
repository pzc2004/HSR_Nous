...
128 隐士（杰帕德） 2pc/4pc 护盾量 +10%/+12%（对方 p2x/p4x SHIELD 标签 BOOST 同值
                   ...
                   4pc 后半「持盾友方 crit_dmg 15%」**已收编（2026-09-23——
                   has_shield 逐目标判定 + on_turn_start 懒扫描双钩）→ 自盾场三方比等
                   （batch3 映射本波实现）
...
S16 126 助力爆发窗口（对方 enabled 恒开含当次大招=建模近似；我方 on_ultimate
    ...
    本波 126 条目改 S16）
~~S17 128 持盾暴伤~~ **已收官（2026-09-23，has_shield 逐目标判定 + on_turn_start
    懒扫描双钩——自盾场三方全等 CD 0.65 档）**
...
S-消费端【已收官 2026-09-22】21008 hook 承载 DoT tick 不吃 dot_dmg_boost 桶——
    ...
    〔2026-09-23 续：卢卡裂伤已迁声明式 dot 通道——其 hook action_type "dot" 声明
    随之退役，本枚举减一；dot_dmg_boost 桶由声明式 dot_tick 快照原生消费〕
