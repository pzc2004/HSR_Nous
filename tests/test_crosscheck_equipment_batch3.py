...
残部续波——本批 35 光锥（3/4 星功能件为主）+ 6 遗器映射表，每件等式场景三方比对
（我方引擎 vs 对方 vs 手算，rel_tol 1e-4）+ 结构差钉倍数。

计数链勘正注（2026-09-24）：真实计数=35 光锥测试实体（35 测试类/43 例）——首提交
0f4c940 提交信息「42 件」与 docstring 旧文「37 光锥 + 6 遗器」均虚（提交信息不可改，
留作历史注记）；6 遗器（107/119/121/125/126/128）当时只写映射表无测试实体
（faa4b1e 注记在案，测试实体在 batch4 补齐）。
