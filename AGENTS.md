...
- **打标 DAG 批量调度**：`scripts/annotator.sh batch [--ids|--limit|--include-anchors|--workers N]`（ops/annotator——单角色 DAG：data_pull→crosscheck→社区层→evidence→draft→compile/smoke/golden_diff 三闸内环→finalize；金样对拍=白值/技能 id 集/scaling 全表对官方数据锚机械闸；runs_root 断点续跑，汇总 `_batch_summary.json`；staging 候选包合并走人工闸；--workers=多角色外层并行，宽度受 LLM 客户端 key 并发帽统一限流）
