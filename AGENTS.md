...
- **打标 DAG 批量调度**：`scripts/annotator.sh batch [--kind character|light_cone|relic] [--ids|--limit|--include-anchors|--workers N]`（ops/annotator——单实体 DAG：data_pull→crosscheck→社区层→evidence→draft→compile/smoke/golden_diff 三闸内环→finalize；角色版金样对拍=白值/path canonical/技能 id 集/scaling 全表对官方数据锚机械闸；装备分支（光锥/遗器，equipment_nodes）链形同构——draft LLM 只产 hooks 块、数值区机械合并自生成器草稿（杜绝面板幻觉），金样=白值/叠影行/2pc 属性对数据锚，drift 指路重跑生成器；runs_root/<kind>s/<id>/ 断点续跑（角色在 runs_root/<cid>/），汇总 `_batch_summary{,_kind}.json`；staging 候选包合并走人工闸；--workers=多实体外层并行（热更：`~/.config/hsr_nous/annotator_live_config.json` 的 `batch_workers` 键——运行中调档免重启，升档补位/降档排干），宽度受 LLM 客户端 key 并发帽统一限流）
