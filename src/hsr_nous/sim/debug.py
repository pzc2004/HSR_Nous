...
class DebugController:
    ...
    def decisions(self) -> Dict[int, str]:
        """决策簿（turn_count → action_id）：可落盘的最小轨迹。"""
        return {t: a for t, a in self._cell["record"]}
    ...
    def goto_turn(self, n: int) -> Dict[str, Any]:
        ...
        record[:] = [(t, a) for t, a in record if t < n]
        ...
        self._cell["replay_queue"] = [a for t, a in record if cp[0] <= t < n]
        try:
            last: Optional[Dict[str, Any]] = None
            while not self._done and self.state.turn_count < n:
                last = self.step_turn()
    ...
    def action_bar(self, n: int = 10) -> List[Dict[str, Any]]:
        """行动条预览（只读）：[(actor, 回合类型, 预计时刻)]，前 n 条。"""
        self.engine.setup()
        assert self.engine.scheduler is not None
        return [
            {"actor_id": a.actor_id, "name": a.name, "kind": kind, "eta": round(eta, 1)}
            for a, kind, eta in self.engine.scheduler.preview(n)
        ]
