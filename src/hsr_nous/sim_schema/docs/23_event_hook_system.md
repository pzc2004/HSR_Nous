...
| `on_hp_decrease` | actor HP 降低时 | `self` / `team` | `amount`、`source`、`reason`、`target`、`damage_type`（仅 `reason == 'hit'` 受击族发射携带——action 伤害与 hook 伤害两发射点同带，真伤为 `"true"`；结界/印记"非真伤才触发"过滤闸——昔涟 141502 真伤防递归。dot/break/drain/set_hp 族无此字段）、`action_type`（同为 `'hit'` 族携带——action 伤害=行动类别、hook 非真伤伤害=伪行动类别 `follow_up`/`additional`；"指定技能造成的伤害"族过滤源——遐蝶 140701 星魂 1 四技限定。真伤/dot/break/drain/set_hp 族无此字段）、`is_critical`（同为 `'hit'` 族携带——暴击门控族挂载点：丹恒 100202 战技暴击施加减速；与 `before_take_damage`/`after_being_hit` 同名字段同义。期望模式恒 `false`——期望暴击区已并入伤害无离散暴击，掷骰模式读判定 trace） | emit | 已登记 |
