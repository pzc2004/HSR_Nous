...
| `on_hp_decrease` | actor HP 降低时 | `self` / `team` | `amount`、`source`、`reason`、`target`、`damage_type`（仅 `reason == 'hit'` 受击族发射携带——action 伤害与 hook 伤害两发射点同带，真伤为 `"true"`；结界/印记"非真伤才触发"过滤闸——昔涟 141502 真伤防递归。dot/break/drain/set_hp 族无此字段） | emit | 已登记 |
