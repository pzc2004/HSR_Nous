...
| summons 块校验 | 角色模板 `summons:` 块（12_summon v1 已落地）：每个召唤物定义做已知键 diff（`name`/`inheritance`/`base_stats`/`capabilities`/`actions`/`hooks`）、capabilities 键词表 + bool 闸、inheritance 合法值（`"full"`/`"none"`/stat 字段名列表，none 时 base_stats 必填 hp）、actions/hooks 与角色模板同一编译闸 | `含未知键 'x'` / `inheritance 非法值` / `须为 bool` |
